...
v1=(1+0.5*rand(1,4))*velocity_init;  %初始速度~U（1~1.5）；rand（1,4）生成1row*4collumn的矩阵，元素为Uniform（0,1）的随机数
v = repmat(v1,2,1); %replicate v1 by 2*1，即v为2*4矩阵
...
beta = [pi/6+0.2];
%v_change = v(:,1).*[cos(beta),sin(beta)]'%速度变化
