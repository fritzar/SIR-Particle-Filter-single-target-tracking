...
% [initx初始状态位置设置,x,x_c] = GenerateTarget(Target_number,velocity_init,axisX,axisY,Total_time,F,Q);
...
%% 
%预分配空间
E_target = zeros(7,Total_time,Monte,length(SNR),length(NpN)); %目标状态估计
Target_p_error = zeros(Total_time,Monte,length(SNR),length(NpN)); %位置误差平方差
error_P = zeros(Total_time,length(SNR),length(NpN)); %位置误差均方根误差RMSE(Root Mean Square error), root(mean(Target_P_error))
...
%                 hold off
            ...
            [E_target_state]=standard_SIR(Np,initx,Re_x,Re_y,numX,numY,Total_time,xy_data,Sigma_noise,A,F,Q);%输入粒子滤波器
            %F、Q为纯粹粒子滤波KP加上的
